...
<header class="sticky top-0 z-50 bg-white">
  <nav aria-label="Global" class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
    ...
    <el-popover-group class="hidden lg:flex lg:gap-x-12">
      ...
      <a href="{{ route('welcome') }}" class="text-sm/6 font-semibold text-gray-900">Inicio</a>
      <a href="{{ route('sobre-nosotros') }}" class="text-sm/6 font-semibold text-gray-900">Sobre Nosotros</a>
    </el-popover-group>
  ...
  @auth
    <div class="relative">
      <button 
        ...
        class="flex items-center gap-x-3 rounded-full border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm transition hover:border-gray-300 hover:bg-gray-50"
      >
      ...
      <el-popover 
        ...
        anchor="bottom-end" 
        popover 
        class="mt-3 w-56 overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-gray-900/5 transition transition-discrete"
      >
        ...
        <div class="p-2">
          <a 
            ...
            class="block rounded-xl px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
          >
            Mi Perfil
          </a>

          <a 
            href="{{ route('mi.configuracion') }}" 
            class="block rounded-xl px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
          >
            Configuración
          </a>
          ...
          <form method="POST" action="{{ route('logout') }}" class="mt-1">
            ...
            <button 
              type="submit" 
              class="block w-full cursor-pointer rounded-xl px-4 py-2 text-left text-sm font-semibold text-red-600 transition-all hover:bg-gray-50 hover:text-red-700"
            >
              Cerrar sesión
            </button>
          </form>
  ...
  @else
    <div class="flex items-center gap-x-3">
      <a 
        href="{{ route('login') }}" 
        class="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm transition hover:border-gray-300 hover:bg-gray-50"
      >
        Iniciar Sesión
      </a>
      ...
      <div tabindex="0" class="fixed inset-0 focus:outline-none">
        <el-dialog-panel class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white p-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div class="flex items-center justify-between">
            <a href="{{ route('welcome') }}" class="-m-1.5 p-1.5">
              <span class="sr-only">Your Company</span>
              <img src="{{ asset('storage/img/logo.png') }}" alt="logo" class="h-12 w-auto" />
            </a>
          ...
          <div class="mt-6 flow-root">
            <div class="-my-6 divide-y divide-gray-500/10">
              <div class="space-y-2 py-6">
                ...
                <a href="{{ route('welcome') }}" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-gray-900 hover:bg-gray-50">Inicio</a>
                <a href="{{ route('sobre-nosotros') }}" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-gray-900 hover:bg-gray-50">Sobre Nosotros</a>
              </div>
              <div class="py-6">
                @auth
                  <div class="-mx-3 mb-3 px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    Usuario: {{ auth()->user()->name }}
                  </div>
                  <a href="{{ route('mi.perfil') }}" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-gray-900 hover:bg-gray-50">Mi Perfil</a>
                  <a href="{{ route('mi.configuracion') }}" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-gray-900 hover:bg-gray-50">Configuración</a>
                  
                  <form method="POST" action="{{ route('logout') }}" class="mt-4">
                    @csrf
                    <button type="submit" class="-mx-3 block w-full cursor-pointer text-left rounded-lg px-3 py-2.5 text-base/7 font-semibold text-red-600 transition-all hover:bg-gray-50 hover:text-red-700">
                      Cerrar sesión
                    </button>
                  </form>
                @else
                  ...
                  @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="-mx-3 block rounded-lg px-3 py-2.5 text-base/7 font-semibold text-gray-900 hover:bg-gray-50">Registrarse</a>
                  @endif
                @endauth
              </div>
            </div>
